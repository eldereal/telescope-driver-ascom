...
namespace Me.Eldereal.ElmsRemoteDriverBase
{
    ...
    public class Commands
    {
        ...
        public static byte[] CommandFocusMove(int steps)
        {
            byte[] r = new byte[5];
            r[0] = (byte)CommandByte.FocusMove;
            Array.Copy(BitConverter.GetBytes(IPAddress.HostToNetworkOrder(steps)), 0, r, 1, 4);
            return r;
        }

        public static byte[] CommandFocusAbort()
        {
            return new byte[] { (byte)CommandByte.FocusAbort };
        }
